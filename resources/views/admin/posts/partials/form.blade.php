    ...
    <div class="form-group">
        ...
        @foreach ($tags as $key => $tag)
            <label class="mr-2">
                <input class="mr-1" name="tags[]" id="tags" type="checkbox" value="{{ $tag->id }}" @isset($post->tags) @if(in_array($tag->id, $post->tags->pluck('id')->toArray(), )) checked @endif @endisset>{{ $tag->name}}
            </label>
        @endforeach
        @error('tags')
            <br>
